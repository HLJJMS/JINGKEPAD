package smjj.pureclass_pad.xorzlistview.xrefreshview.recyclerview;
...
import smjj.pureclass_pad.xorzlistview.xrefreshview.IFooterCallBack;
...
//            notifyItemRemoved(getItemCount() - 1);
    ...
    public View setHeaderView(@LayoutRes int id, RecyclerView recyclerView) {
        ...
        if (!resourceTypeName.contains("layout")) {
            throw new RuntimeException(context.getResources().getResourceName(id) + " is a illegal layoutid , please check your item_lesson id first !");
        }
        RecyclerView.LayoutManager layoutManager = recyclerView.getLayoutManager();
        if (layoutManager != null && layoutManager instanceof GridLayoutManager) {
            GridLayoutManager gridLayoutManager = (GridLayoutManager) layoutManager;
